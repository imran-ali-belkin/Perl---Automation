...
#
print "========================================================\n";
print "The C7 GPON Video to E7 GPON Video Script Creator\n";
...
print "1: Logs into a C7 and pulls video subs from a GPON card\n";
print "2: Creates a text file that has the E7 video sub syntax\n";
...
print "\n\n";
print "========================================================\n";
print "Enter the C7 IRC Card Node-Shelf-Slot you want to retrieve:\n";
print "      Example: N1-1-20\n";
print ">";
my $C7_IRC_card = <STDIN>;
chomp($C7_IRC_card);

print "\n\n";
print "========================================================\n";
print "Enter the E7 Video Service Tag Action Name (example: \@Video):\n";
print ">";
my $E7_Video_Service_Tag_Action_Name = <STDIN>;
chomp($E7_Video_Service_Tag_Action_Name);


print "\n\n";
print "========================================================\n";
print "Enter the E7 Video Multicast Profile (example: \@Multicast):\n";
print ">";
my $E7_Multicast_Profile = <STDIN>;
chomp($E7_Multicast_Profile);
...
$t->timeout(undef);
$t->send("RTRV-VID-SUB::$C7_gpon_card-all::::RTRAID=$C7_IRC_card;");
print "=============================\n";
print "I am retrieving all video subs on $C7_gpon_card\n";  
print "Please be patient.\n";
print "=============================\n";

$t->waitfor('/^;/ms');
print "======================================\n";
print "OK, done retrieving the video subs on $C7_gpon_card\n";
print "======================================\n";
...
my $DESTINATION_FILE = 'E7_SYNTAX_FOR_VIDEOSUBS_' . $C7_gpon_card . '.txt';
...
while ($line = <$fh> )
{
  chomp($line);
		...
		# "N130-1-1-1-4-1::RTRAID=N105-1-20,CHANCNT=4,VIDBW=38700,VIDLOANBW=2000,BWINUSE=7414,CHANIP=224.2.2.201&239.1.0.5&239.1.0.62&239.255.5.3,STBIP=10.100.47.5&10.100.44.250&10.100.36.164&10.100.35.166"
		 if 
		 (
		 $line =~ m!(N\d+-\d+-\d+-(\d+)-(\d+)-(\d+))::RTRAID=N\d+-\d+-\d+,CHANCNT=(\w+),!
		 )
		 {
			# $1 is the entire AID
			# $2 is the OLTG port
			...
			if ( $2 eq '1')
			{ 
				print {$fh_E7_CREATES} ("add eth-svc Video1 to-ont-port $gpon_port_1_prefix$temp_ont/g$4 bw-profile $5 svc-tag-action $E7_Video_Service_Tag_Action_Name mcast-profile $E7_Multicast_Profile admin-state enabled\n");
				$xcon_written_port_1++;
				$xcon_written++;
			}
			# If PON port 2
			...
			{ 
				print {$fh_E7_CREATES} ("add eth-svc Video1 to-ont-port $gpon_port_2_prefix$temp_ont/g$4 bw-profile $5 svc-tag-action $E7_Video_Service_Tag_Action_Name mcast-profile $E7_Multicast_Profile admin-state enabled\n");
				$xcon_written_port_2++;
				$xcon_written++;
			}
			# If PON port 3
			...
			{ 
				print {$fh_E7_CREATES} ("add eth-svc Video1 to-ont-port $gpon_port_3_prefix$temp_ont/g$4 bw-profile $5 svc-tag-action $E7_Video_Service_Tag_Action_Name mcast-profile $E7_Multicast_Profile admin-state enabled\n");
				$xcon_written_port_3++;
				$xcon_written++;
			}			
			# If PON port 4
			...
			{ 
				print {$fh_E7_CREATES} ("add eth-svc Video1 to-ont-port $gpon_port_4_prefix$temp_ont/g$4 bw-profile $5 svc-tag-action $E7_Video_Service_Tag_Action_Name mcast-profile $E7_Multicast_Profile admin-state enabled\n");
				$xcon_written_port_4++;
				$xcon_written++;
			}			
...
print "Video Subs found on port 1:\t $xcon_written_port_1\n";
print "Video Subs found on port 2:\t $xcon_written_port_2\n";
print "Video Subs found on port 3:\t $xcon_written_port_3\n";
print "Video Subs  found on port 4:\t $xcon_written_port_4\n";

print" Total data entries written out to E7 file: $xcon_written\n";

print" Total video subs not matched: $xcon_not_matched\n";
