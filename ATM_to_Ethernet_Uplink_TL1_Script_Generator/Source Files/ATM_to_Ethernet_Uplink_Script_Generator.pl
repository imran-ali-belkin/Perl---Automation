  	...
  	if ( $line =~ m!PATH=PROT!) {
		...
		if ( $line =~ m!BWC=\d+,!){
				...
				if ( $line =~ m!(N\d+-\d+-\d+-\d+-VP\d+-VC\d+),(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
						...
						{
 							print {$fh2} ("ENT-VLAN-IF::$2::::VLAN=$user_specified_vb" . "-VLAN$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,PORTTYPE=EDGE,CVID=UNTAGGED,SVID=$user_specified_vlan,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4,BWC=$6;\n");
						...
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::BRIDGE=$user_specified_vb,VLAN=$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,PORTTYPE=EDGE,CVID=UNTAGGED,SVID=$user_specified_vlan,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4,BWC=$6;\n");
							$counter_DSL_classic_creates++;	
							$cross_connects_create_created++;
						}	
				}
		...
		# End of Bandwidth Constraint logic
				...
				if ( $line =~ m!(N\d+-\d+-\d+-\d+-VP\d+-VC\d+),(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
						...
						if ( $5 eq 'WKG' ) {
 						...
 						if ( $user_specified_sw_version eq '6.1' ) 
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::VLAN=$user_specified_vb" . "-VLAN$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,PORTTYPE=EDGE,CVID=UNTAGGED,SVID=$user_specified_vlan,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4;\n");
							$counter_DSL_classic_creates++;					
							$cross_connects_create_created++;
						}
						elsif ( $user_specified_sw_version eq '7.0' || $user_specified_sw_version eq '7.2')
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::BRIDGE=$user_specified_vb,VLAN=$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,PORTTYPE=EDGE,CVID=UNTAGGED,SVID=$user_specified_vlan,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4;\n");
							$counter_DSL_classic_creates++;					
							$cross_connects_create_created++;
						}	
 				}
