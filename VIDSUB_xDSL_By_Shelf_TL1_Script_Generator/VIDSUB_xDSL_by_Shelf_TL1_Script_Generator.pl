...
# Requirements
#
# VIDSUB_xDSL_By_Shelf_Script_Generator_Script_Generator

# Script Generator Name: VIDSUB_xDSL_By_Shelf_Script_Generator_Script_Generator.exe
...
# Script generator questions to ask the user:
  ...
  # TL1 Command:
  # RTRV-VID-SUB::N15-1-ALL::::RTRAID=N1-1-7;  
...
# 1. VIDSUB_DELETES_FOR_Nx-x.txt  <-- Where Nx-x = Ques #2.
# 2. VIDSUB_CREATES_FOR_Nx-x.txt  <-- Where Nx-x = Ques #2.

# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# File 1: VIDSUB_DELETES_FOR_Nx-x.txt
  # TL1 Command:
  # DLT-VID-SUB::N15-1-1-1-CH0::::RTRAID=N1-1-7;
...
# File 2: VIDSUB_CREATES_FOR_Nx-x.txt
  ...
  # TL1 Command:
  # ENT-VID-SUB::N15-1-1-1-CH0::::RTRAID=N1-1-7,CHANCNT=3,VIDBW=14600,VIDLOANBW=10000;
...
#
print "========================================================\n";
print "VIDSUB_xDSL_By_Shelf_TL1_Script_Generator\n";
...
my $CREATES_FILE = 'VIDSUB_CREATES_FOR_' .$C7_xDSL_shelf. '.txt';
open $fh_C7_CREATES, ">", $CREATES_FILE or die "Couldn't open $CREATES_FILE: $!\n";

my $fh_C7_DELETES;		# File handle for DELETES
my $DELETES_FILE = 'VIDSUB_DELETES_FOR_' .$C7_xDSL_shelf. '.txt';
...
while ($line = <$fh> )
{
  ...
  if 
  (
	$line =~ m!(N\d+-\d+-\d+-\d+-CH0)::RTRAID=N\d+-\d+-\d+,CHANCNT=(\w+),VIDBW=(\w+),VIDLOANBW=(\w+)[,"]!  
   )
   {
	 my $dest = 		$1;
	 my $chancnt =  	$2;
