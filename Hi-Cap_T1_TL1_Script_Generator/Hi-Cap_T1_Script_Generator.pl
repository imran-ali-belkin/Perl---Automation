...
print "\n\n\n\n\n\n";
print "========================================================\n";
print "The C7 T1 Script Creator\n";
print "========================================================\n";
print "\n\n";
print "This script does these three things:\n";
print "1: Logs into a C7 and pulls T1 cross-connect data\n";
...
print "\n\n";
print "========================================================\n";
print "Enter the Node and Shelf of the T1 hi-caps\n";
...
# T1 Script Creator Specific Logic
print "=============================\n";
print "I am retrieving all of the T1 cross-connects on $user_shelf \n";  
print "Please be patient.\n";
print "=============================\n";
$t->timeout(undef);
$t->send("RTRV-CRS-T1::$user_shelf-all::::scope=ntwk;");
$t->waitfor('/^;/ms');
...
my $DESTINATION_FILE = 'CREATES_FOR_' . $user_shelf . '.txt';
my $DESTINATION_FILE_2 = 'DELETES_FOR_' . $user_shelf . '.txt';
my $line;
my $fh2;		# CREATES
my $fh3;		# DELETES
...
while ($line = <$fh> )
{
  chomp($line);
		  ...
		  # BWCs used with T1 hi-caps
		  if ($line =~ (/BWC=/) ){
			  # Logic to match on T1 to T1. or on T1 to ONT, or ONT to ONT, or ONT to T1
			 if (
				...
				)
				{
					...
					if ( $2 eq $4 )
					{
						print {$fh2} ("ENT-CRS-T1::$1,$3::::PATH=UNPROT,BWC=$5;\n");
					}
					else 
					{
						print {$fh2} ("ENT-CRS-T1::$1,$3::::PATH=BOTH,BWC=$5;\n"); 
					}
			  ...
			  # Logic to match on T1 to T1. or on T1 to ONT, or ONT to ONT, or ONT to T1
				 if (
				    ...
				    )			
					{
						...
						if ( $2 eq $4 )
						{
							print {$fh2} ("ENT-CRS-T1::$1,$3::::PATH=UNPROT;\n");
						}
						else 
						{
							print {$fh2} ("ENT-CRS-T1::$1,$3::::PATH=BOTH;\n");
						}
...
while ($line = <$fh> )
{
  ...
  if ($line =~ /PATH=WKG/ || $line =~ /PATH=UNPROT/ ){
	  ...
	  # Logic to match on T1 to T1. or on T1 to ONT
     if (
			$line =~ m!(N\d+-\d+-\d+-\d+),(N\d+-\d+-\d+-\d+)! || 
			$line =~ m!(N\d+-\d+-\d+-\d+),(N\d+-\d+-\d+-\d+-\d+-\d+)! || 
			$line =~ m!(N\d+-\d+-\d+-\d+-\d+-\d+),(N\d+-\d+-\d+-\d+-\d+-\d+)! ||
