...
print "\n\n\n\n\n\n";
print "==============================================================================\n";
print "The C7 ATM To Ethernet by Shelf Script Creator\n";
...
print "===============================================================================\n";
print "Enter the Shelf you want the Subscribers pulled for:\n";
print "Make sure to get this correct or else I will not be able to retrieve properly\n";
print "For example, if the subscribers are in shelf N2-1 then type N2-1 and hit enter:\n";
print "\n";
print ">";
...
print "===============================================================================\n";
print "Enter the ATM traffic profile that the Subscribers are mapped to:\n";
 print "For example, if the subscribers are using UBR then type UBR and hit enter:\n";
print "\n";
print ">";
my $user_ATM_traffic_profile = <STDIN>;
chomp($user_ATM_traffic_profile);
...
$t->timeout(undef);
$t->send("RTRV-CRS-VC::$user_data_uplink-all::::TRFPROF=$user_ATM_traffic_profile,SCOPE=ntwk;");
print "=============================\n";
...
print "=====================================================\n";
print "Now I am going to read in the TL1 text file for $user_data_uplink\n";
...
my $DESTINATION_FILE = 'CREATES_VLANIFs_FOR_' . $user_data_uplink . '.txt';
my $DESTINATION_FILE_2 = 'DELETES_ATM_XCONS_FOR_' . $user_data_uplink . '.txt';
  	...
  	if ( $line =~ m!PATH=PROT!) {
		...
		if ( $line =~ m!BWC=\d+,!){
				...
				if ( $line =~ m!(N\d+-\d+-\d+-\d+-VP\d+-VC\d+),(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
						...
						{
 							print {$fh2} ("ENT-VLAN-IF::$2::::VLAN=$user_specified_vb" . "-VLAN$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,DIRN=DOWN,PORTTYPE=EDGE,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4,BWC=$6;\n");
						...
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::BRIDGE=$user_specified_vb,VLAN=$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,PORTTYPE=EDGE,DIRN=DOWN,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4,BWC=$6;\n");
							$counter_DSL_classic_creates++;	
							$cross_connects_create_created++;
						}	
				}
				...
				#DS3 oriented ATM uplink with ONT access with a BWC:
						...
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::BRIDGE=$user_specified_vb,VLAN=$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,DIRN=DOWN,PORTTYPE=EDGE,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4,BWC=$6;\n");
							$counter_ONT_classic_creates++;
							$cross_connects_create_created++;
						}	
 				}		
		...
		# End of Bandwidth Constraint logic
				...
				if ( $line =~ m!(N\d+-\d+-\d+-\d+-VP\d+-VC\d+),(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
						...
						if ( $5 eq 'WKG' ) {
 						...
 						if ( $user_specified_sw_version eq '6.1' ) 
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::VLAN=$user_specified_vb" . "-VLAN$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,DIRN=DOWN,PORTTYPE=EDGE,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4;\n");
							$counter_DSL_classic_creates++;					
							$cross_connects_create_created++;
						}
						elsif ( $user_specified_sw_version eq '7.0' || $user_specified_sw_version eq '7.2')
						{
							print {$fh2} ("ENT-VLAN-IF::$2::::BRIDGE=$user_specified_vb,VLAN=$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,DIRN=DOWN,PORTTYPE=EDGE,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4;\n");
							$counter_DSL_classic_creates++;					
							$cross_connects_create_created++;
						}	
 				}
				...
				elsif ($line =~ m!(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+),(N\d+-\d+-\d+-IMA\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),!)
				{
						...
						if ( $user_specified_sw_version eq '6.1' ) 
						{
							print {$fh2} ("ENT-VLAN-IF::$1::::VLAN=$user_specified_vb" . "-VLAN$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,DIRN=DOWN,PORTTYPE=EDGE,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4;\n");
							$counter_IMA_uplink_creates++;
							$cross_connects_create_created++;
						}
						elsif ( $user_specified_sw_version eq '7.0' || $user_specified_sw_version eq '7.2')
						{
							print {$fh2} ("ENT-VLAN-IF::$1::::BRIDGE=$user_specified_vb,VLAN=$user_specified_vlan,ARP=N,DHCPDIR=CLIENT,PPPOESUB=Y,DOS=Y,DIRN=DOWN,PORTTYPE=EDGE,CVID=UNTAGGED,PRIO=0,RCVID=NONE,PATH=$path_info,TRFPROF=$3,BCKPROF=$4;\n");
							$counter_IMA_uplink_creates++;
							$cross_connects_create_created++;
						}	
 				}
