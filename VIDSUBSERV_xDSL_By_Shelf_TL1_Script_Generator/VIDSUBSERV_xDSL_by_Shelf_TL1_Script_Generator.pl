...
# Requirements
#
# VIDSUBSERV_xDSL_By_Shelf_Script_Generator_Script_Generator

# Script Generator Name:	VIDSUBSERV_xDSL_By_Shelf_Script_Generator_Script_Generator.exe

# Script generator questions to ask the user:
  # 1. Please enter either the IP address or hostname of the C7 you want to reach:
  # 2. Enter the node-shelf that you want to retrieve on (example N1-1 or N2-1):
  # 3. Enter the working IRC location (example: N1-1-20)
  
  # TL1 Command:
  # RTRV-CRS-VIDVC::N15-1-ALL::::IRCAID=N1-1-7,APP=VIDSUBSVC;
  
  # Syntax:
  # RTRV-CRS-VIDVC::[Ques #2]-ALL::::IRCAID=[Ques #3],APP=VIDSUBSVC;  

  # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Output:

 # "N15-1-11-PP1-VP0-VC1010,N15-1-1-1-CH0-VP0-VC40::TRFPROF=50,PATH=UNPROT,APP=VIDSUBSVC,CONSTAT=NORMAL"
 
 # Pattern match on all CRS (they should all be UNPROT).
 
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# Create 2 output files:

# 1. VIDSUBSERV_DELETES_FOR_Nx-x.txt  <-- Where Nx-x = Ques #2.
# 2. VIDSUBSERV_CREATES_FOR_Nx-x.txt  <-- Where Nx-x = Ques #2.

# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# File 1: VIDSUBSERV_DELETES_FOR_Nx-x.txt

# TL1 Command:
 
  # DLT-CRS-VIDVC::N15-1-11-PP1-VP0-VC1010,N15-1-1-1-CH0-VP0-VC40::::IRCAID=N1-1-7;
...
#
#  TL1 Command:
#  ENT-CRS-VIDVC::N15-1-11-PP1-VP0-VC1010,N15-1-1-1-CH0-VP0-VC40::::IRCAID=N1-1-7,TRFPROF=50,PATH=UNPROT; 
#
#  Pattern match:
...
#
print "========================================================\n";
print "VIDSUBSERV_xDSL_By_Shelf_Script_Generator\n";
...
print "1: Logs into a C7 and pulls VIDSUBSERV xDSL info\n";
print "2: Creates two text files VIDSUBSERV DELETES and CREATES\n";
...
$t->timeout(undef);
$t->send("RTRV-CRS-VIDVC::" .$C7_xDSL_shelf. "-ALL::::IRCAID=" .$irc_location. ",APP=VIDSUBSVC;");


  
print "=============================\n";
print "I am retrieving all xcons with APP=VIDSUBSVC on " .$C7_xDSL_shelf. "\n";  
...
my $CREATES_FILE = 'VIDSUBSERV_CREATES_FOR_' .$C7_xDSL_shelf. '.txt';
open $fh_C7_CREATES, ">", $CREATES_FILE or die "Couldn't open $CREATES_FILE: $!\n";

my $fh_C7_DELETES;		# File handle for DELETES
my $DELETES_FILE = 'VIDSUBSERV_DELETES_FOR_' .$C7_xDSL_shelf. '.txt';
...
while ($line = <$fh> )
{
  ...
  if 
  (
	$line =~ m!(N\d+-\d+-\d+-PP1-VP\d+-VC\d+),(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+)::TRFPROF=(\w+),PATH=(UNPROT),APP=VIDSUBSVC,!  
   )
   {
     my $source = 	$1;
	 my $dest = 	$2;
