...
#
print "========================================================\n";
print "The C7 GPON Data to E7 GPON Data Script Creator\n";
...
while ($line = <$fh> )
{
  chomp($line);
		 ...
		 # N130-1-1-1-3-1-1:2WAY:KEY=N130-1-11-1-1-VP4094-VC5849,TRFPROF=25,BCKPROF=403,PATH=WKG
		 if 
		 (
		 $line =~ m!(N\d+-\d+-\d+-(\d+)-(\d+)-(\d+)-\d+):2WAY:KEY=N\d+-\d+-\d+-\d+-\d+-VP\d+-VC\d+,TRFPROF=(\w+),BCKPROF=(\w+),PATH=WKG,!
		 ||
		 $line =~ m!(N\d+-\d+-\d+-(\d+)-(\d+)-(\d+)-\d+):2WAY:KEY=N\d+-\d+-\d+-\d+-\d+-VP\d+-VC\d+,TRFPROF=(\w+),BCKPROF=(\w+),PATH=UNPROT,!		 
		 )
		 {
			# $1 is the entire AID
			# $2 is the OLTG port
