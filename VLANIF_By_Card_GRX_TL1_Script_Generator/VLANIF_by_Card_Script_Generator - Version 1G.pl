...
print "\n\n\n\n\n\n";
print "========================================================\n";
print "The C7 VLANIF By Card Script Creator\n";
print "\tVersion:  1G\n";
print "\tUpdated:  3-25-2013\n";
print "========================================================\n";
print "\n\n";
print "This script does these three things:\n";
print "1: Logs into a C7 and pulls VLANIF cross-connect data for a\n";
print "   user-specified access card (i.e. DSL or GPON, etc).\n";
...
print "\n\n";
print "========================================================\n";
print "Enter the Specific Access Card (DSL or GPON, etc) that you want the VLANIFs created for.\n";
print "For example, if the DSL/GPON card is N2-1-1 and need\n";
print "then type N2-1-1 and hit the enter key:\n";
print ">";
my $user_shelf = <STDIN>;
chomp($user_shelf);
...
$t->timeout(undef);
$t->send("RTRV-VLAN-IF::$user_shelf-ALL::::VLAN=$user_data_uplink-VLAN$user_data_vlan,DETAILS=Y;");
$t->waitfor('/^;/ms');


# Retrieving all VLANIFs in 7.0/7.2 mode
# rtrv-vlan-if::ALL::::VLAN=12,BRIDGE=N1-1-VB1,DETAILS=Y;
$t->send("RTRV-VLAN-IF::$user_shelf-ALL::::VLAN=$user_data_vlan,BRIDGE=$user_data_uplink,DETAILS=Y;");
$t->waitfor('/^;/ms');
...
my $DESTINATION_FILE = 'CREATES_FOR_' . $user_shelf . "_" . $user_data_uplink . "_VLAN $user_data_vlan.txt";
my $DESTINATION_FILE_2 = 'DELETES_FOR_' . $user_shelf . "_" . $user_data_uplink . "_VLAN $user_data_vlan.txt";
my $line;
my $fh2;		# CREATES
my $fh3;		# DELETES
	...
	if ($network_software_version eq '7.0' || $network_software_version eq '7.2') 
	{
		...
		{						
				...
				# rtrv-vlan-if::ALL::::VLAN=12,BRIDGE=N1-1-VB1,DETAILS=YES;
						...
						# 7.0 Ethernet uplink with DSL access and STPST= and ROLE= params
						# "N105-4-11-1-CH0-VP8-VC35:N101-1-VB1-4785:VLAN=1110,BRIDGE=N101-1-VB1,ARP=N,DHCPDIR=SERVER,OPT82ACT=NONE,IGMP=NONE,PPPOEAC=N,PPPOESUB=Y,LSVID=1110,ENCAP=ETHERNETV2,DOS=N,STP=1D_ADDR,STPCOST=100,STPPRIO=128,DIRN=BOTH,STAGTYPE=CTAG_8100,PORTTYPE=EDGE,STPST=DISCARD,ROLE=DISABLED,TRFPROF=UBR,BCKPROF=UBR,PATH=WKG,CVID=UNTAGGED,SVID=1110,PRIO=0,RCVID=NONE,RXETHBWPROF=NONE,TXETHBWPROF=NONE,"                                                                                                                                                                                                                                                                             
						elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*:VLAN=(\w+),BRIDGE=(\w+-\d+-\w+),ARP=(\w+),DHCPDIR=(\w+),OPT82ACT=(\w+),IGMP=(\w+),PPPOEAC=(\w+),PPPOESUB=(\w+),LSVID=(\w+),ENCAP=(\w+),DOS=(\w+),STP=(\w+),STPCOST=(\w+),STPPRIO=(\w+),DIRN=(\w+),STAGTYPE=(\w+),PORTTYPE=(\w+),STPST=\w+,ROLE=\w+,TRFPROF=(\w+),BCKPROF=(\w+),PATH=(\w+),CVID=(\w+),SVID=(\w+),PRIO=(\w+),RCVID=(\w+),RXETHBWPROF=(\w+),TXETHBWPROF=(\w+)(,|")!)
						{
								if ( $21 eq 'WKG') {
									$new_path = 'BOTH';
								}
