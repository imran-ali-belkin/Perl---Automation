...
#
print "========================================================\n";
print "The C7 to E7 GPON Voice Tag Action Creation Script Creator\n";
print "\tWritten 1-21-2014\n";
...
print "2: Creates a text file that has the E7 Voice Tag Actions\n";
...
print "\n\n";
print "========================================================\n";
print "Enter sip or tdm-gw:\n";
print ">";
my $packet_voice_mode = <STDIN>;
chomp($packet_voice_mode);

print "\n\n";
print "========================================================\n";
print "Enter the E7 Voice Tag Action:\n";
print "(example: \@SIP_Voice or \@TDMGW_Voice)\n";
print ">";
my $voice_tag_action = <STDIN>;
chomp($voice_tag_action);
...
my $DESTINATION_FILE = 'E7_SYNTAX_FOR_Voice_Tag_Action_Creates_' . $C7_gpon_card . '.txt';
...
while ($line = <$fh> )
{
  ...
  if ($line =~ /ONTNUM=/ ){
     ...
     if ($line =~ m!(N\d+-\d+-(\d+)-(\d+)-(\d+))::!)
	 {
		# Determine which GPON port pre-fix to use
		# I.E. if GPON Port 1 then use the user-specified port 1 prefix
		# I.E. if GPON Port 2 then use the user-specified port 2 prefix
		# ...			
		if ($3 eq '1')
		{
			$temp_port_prefix = $gpon_port_1_prefix;
			$gpon_ont_on_port_1_found++;
		}
		elsif ($3 eq '2')
		{
			$temp_port_prefix = $gpon_port_2_prefix;
			$gpon_ont_on_port_2_found++;
		}
		elsif ($3 eq '3')
		{
			$temp_port_prefix = $gpon_port_3_prefix;
			$gpon_ont_on_port_3_found++;
		}
		elsif ($3 eq '4')
		{
			$temp_port_prefix = $gpon_port_4_prefix;
			$gpon_ont_on_port_4_found++;
		}
		else
		{
			print {$fh_error} ("GPON port pre-fix not matched at: $line \n");
		}
		...
		#  set ont 11303 ip-host sip svc-tag-action @SIP_Voice
		print {$fh_E7_CREATES} ("set ont $temp_port_prefix$temp_ont ip-host $packet_voice_mode svc-tag-action $voice_tag_action\n");
		$ont_written++;
	 }
