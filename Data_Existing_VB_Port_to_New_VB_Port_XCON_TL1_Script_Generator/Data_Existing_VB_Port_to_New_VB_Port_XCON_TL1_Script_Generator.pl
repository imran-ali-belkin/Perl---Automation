...
# based on a user specified VB port starting point (and then it increments from there).
#
...
print "\n\n\n\n\n\n";
print "==========================================================================\n";
print "The C7 6.0 Cross-Connect to VB Port TL1 Script Generator\n";
print "==========================================================================\n";
print "\n\n";
...
print "\n\n";
print "==========================================================================\n"; 
print "Enter the DSL/PON Access Slot:\n";
print "\n";
print "Make sure to get this correct or else I will not be able to retrieve properly\n";
print "For example, if the DSL card is in N1-1-1 then enter N1-1-1:\n";
...
print "\n\n";
print "==========================================================================\n"; 
print "Enter the future Virtual Bridge to start at:\n";
print "\n";
print "For example, if the future Virtual Bridge is N1-1-VB1 then enter N1-1-VB1\n";
print "                                          NODE-SHELF-VB\n";
print "\n";
print ">";
my $user_future_vb = <STDIN>;
chomp($user_future_vb);



print "\n\n";
print "==========================================================================\n"; 
print "Enter the VB port to start at:\n";
print "\n";
print "Enter the starting point at which you want the VB ports numbered\n";
print "For example, if you want to start at VB port 1 then type 1 and hit enter\n";
print "\n";
print "*Note: be sure to keep track of the overall VB port scheme - every time this script\n";
print "is run you need to specify a unique VB port that has not been used yet in any prior\n";
print "runs of this script\n";
print "\n";
print ">";
my $user_vb_port = <STDIN>;
chomp($user_vb_port);
  	...
  	if ( ( $line =~ m!PATH=PROT! ) || ( $line =~ m!TSI! ) ) {
	   ...
	   $counter_cross_connects_found++;
		...
		if ( $line =~ m!BWC=\d+,!)
		{
				...
				if ( $line =~ m!(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
					...
					print {$fh2} ("ENT-CRS-VC::$user_future_vb-$user_vb_port,$1::::PATH=$path_info,TRFPROF=$2,BCKPROF=$3,BWC=$5;\n");
					$counter_DSL_classic_creates++;					
					$counter_cross_connects_created++;					
					$user_vb_port++;
				}
				# VB port to ONT access with a BWC:
				elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-\d+-\d+-\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
  				...
  				elsif ( $line =~ m!(N\d+-\d+-\d+-IMA\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
				...
				elsif ( $line =~ m!(N\d+-\d+-\d+-AP\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				elsif ( $line =~ m!(N\d+-\d+-\d+-GRP\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# Packet access (DS3 Port) with a BWC:
 				elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# Packet access (OC Port) with a BWC:
 				elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),.*BWC=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
		...
		# End of Bandwidth Constraint logic
				...
				# DSL access:
				if ( $line =~ m!(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
					...
					print {$fh2} ("ENT-CRS-VC::$user_future_vb-$user_vb_port,$1::::PATH=$path_info,TRFPROF=$2,BCKPROF=$3;\n"); 
					$counter_DSL_classic_creates++;					
					$counter_cross_connects_created++;
					$user_vb_port++;
				}
				...
				elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-\d+-\d+-\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),!)
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# IMA access:
				elsif ( $line =~ m!(N\d+-\d+-\d+-IMA\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# IMA AP access:
				elsif ( $line =~ m!(N\d+-\d+-\d+-AP\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# DSL GRP access:
				elsif ( $line =~ m!(N\d+-\d+-\d+-GRP\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# DS3 Packet access:
				elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
				...
				# OC Packet access:
				elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-\d+-VP\d+-VC\d+):.*TRFPROF=(\w+),.*BCKPROF=(\w+),.*PATH=(\w+),! )
				{
					if ( $4 eq 'WKG'){
						$path_info = 'BOTH';
					}
