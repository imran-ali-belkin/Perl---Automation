...
# Possible match just to narrow it all down:
  my $new_path ;
	...
	if ($network_software_version eq '6.1' ) 
	{
		...
		{		
				...
				{
					 print {$fh_ERROR}("Not a match for create on a 6.x system: $line\n");
				}
	...
	if ($network_software_version eq '7.0' || $network_software_version eq '7.2') 
	{
		...
		{						
				...
				# rtrv-vlan-if::ALL::::VLAN=12,BRIDGE=N1-1-VB1,DETAILS=YES;
						...
						###########################
						#7.0 Ethernet uplink with VDSL access: 2-5-2013
						###########################
						if ( $line =~ m!(N\d+-\d+-\d+-\d+):.*:VLAN=(\w+),BRIDGE=(\w+-\d+-\w+),ARP=(\w+),DHCPDIR=(\w+),OPT82ACT=(\w+),IGMP=(\w+),PPPOEAC=(\w+),PPPOESUB=(\w+),LSVID=(\w+),ENCAP=(\w+),DOS=(\w+),STP=(\w+),STPCOST=(\w+),STPPRIO=(\w+),DIRN=(\w+),STAGTYPE=(\w+),PORTTYPE=(\w+),TRFPROF=(\w+),BCKPROF=(\w+),PATH=(\w+),CVID=(\w+),SVID=(\w+),PRIO=(\w+),RCVID=(\w+),RXETHBWPROF=(\w+),TXETHBWPROF=(\w+)(,|")!)
						{
								if ( $21 eq 'WKG') {
									$new_path = 'BOTH';
								}
						...
						###########################
						elsif ( $line =~ m!(N\d+-\d+-\d+-\d+-CH0-VP\d+-VC\d+):.*:VLAN=(\w+),BRIDGE=(\w+-\d+-\w+),ARP=(\w+),DHCPDIR=(\w+),OPT82ACT=(\w+),IGMP=(\w+),PPPOEAC=(\w+),PPPOESUB=(\w+),LSVID=(\w+),ENCAP=(\w+),DOS=(\w+),STP=(\w+),STPCOST=(\w+),STPPRIO=(\w+),DIRN=(\w+),STAGTYPE=(\w+),PORTTYPE=(\w+),TRFPROF=(\w+),BCKPROF=(\w+),PATH=(\w+),CVID=(\w+),SVID=(\w+),PRIO=(\w+),RCVID=(\w+),RXETHBWPROF=(\w+),TXETHBWPROF=(\w+)(,|")!)
						{
								if ( $21 eq 'WKG') {
									$new_path = 'BOTH';
								}
						...
						{
							 print {$fh_ERROR}("Not a match for create on a 7.x systems: $line\n");
						}
  ...
  chomp($line);
	...
	if ($network_software_version eq '6.1') 
	{
			...
			{			
					...
					{
						print {$fh_ERROR}("Not a match for delete on a 6.x system: $line\n");
					}
			} # end the VLAN match	 	 
	} # end the 6.1 check
	...
	{  
			...
			{			
					...
					#####################################
					# "N1-1-16-1:N1-1-VB1-16-1:VLAN=622,BRIDGE=N1-1-VB1,ARP=Y,                         DHCPDIR=SERVER,OPT82ACT=NONE,IGMP=NONE,PPPOEAC=N,PPPOESUB=N,LSVID=622,ENCAP=ETHERNETV2,DOS=N,STP=OFF,STPCOST=4,STPPRIO=128,DIRN=BOTH,STAGTYPE=CTAG_8100,PORTTYPE=TRUNK,RXETHBWPROF=NONE,TXETHBWPROF=NONE,"
					...
					{
						print {$fh_ERROR}("Not a match for delete on a 7.x system: $line\n");
					}
