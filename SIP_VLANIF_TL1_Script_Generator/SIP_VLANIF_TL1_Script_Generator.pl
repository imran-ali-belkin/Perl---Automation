  ...
  if ($line =~ m!CALIX,C7,.*(6\.1|7\.0|7\.2)\.!)
	{
		...
		print "\n#######################################\n";
		print "I found $network_software_version\n";
		print "#######################################\n";
		last;
	}
}
	...
	if ($network_software_version eq '7.0' || $network_software_version eq '7.2') 
	{
		...
		{						
				...
				# rtrv-vlan-if::ALL::::VLAN=12,BRIDGE=N1-1-VB1,DETAILS=YES;
						...
						###########################
						#7.0 Ethernet uplink with SIP lines:
						###########################
						# "N4-1-5-1-11-SIP1:N12-1-VB1-4:VLAN=30,BRIDGE=N12-1-VB1,ARP=N,DHCPDIR=CLIENT,OPT82ACT=NONE,IGMP=NONE,PPPOEAC=N,PPPOESUB=N,LSVID=30,ENCAP=ETHERNETV2,DOS=Y,STP=OFF,STPCOST=100,STPPRIO=128,DIRN=BOTH,STAGTYPE=CTAG_8100,PORTTYPE=EDGE,TRFPROF=SIP_20MS3WAY,BCKPROF=SIP_20MS3WAY,PATH=WKG,CVID=UNTAGGED,SVID=30,PRIO=0,RCVID=NONE,RXETHBWPROF=NONE,TXETHBWPROF=NONE,MCASTPROF=NONE"
						if ( $line =~ m!(N\d+-\d+-\d+-\d+-\d+-SIP\d+):.*:VLAN=(\w+),BRIDGE=(\w+-\d+-\w+),ARP=(\w+),DHCPDIR=(\w+),OPT82ACT=(\w+),IGMP=(\w+),PPPOEAC=(\w+),PPPOESUB=(\w+),LSVID=(\w+),ENCAP=(\w+),DOS=(\w+),STP=(\w+),STPCOST=(\w+),STPPRIO=(\w+),DIRN=(\w+),STAGTYPE=(\w+),PORTTYPE=(\w+),TRFPROF=(\w+),BCKPROF=(\w+),PATH=(\w+),CVID=(\w+),SVID=(\w+),PRIO=(\w+),RCVID=(\w+),RXETHBWPROF=(\w+),TXETHBWPROF=(\w+)(,|")!)
						{
								if ( $21 eq 'WKG') {
									$new_path = 'BOTH';
								}
  ...
  chomp($line);
	...
	{  
			...
			{			
					...
					#####################################
					# 7.0 Ethernet Uplink with SIP access:
					#####################################					
					# "N4-1-5-1-11-SIP1:N12-1-VB1-4:VLAN=30,BRIDGE=N12-1-VB1,ARP=N,DHCPDIR=CLIENT,OPT82ACT=NONE,IGMP=NONE,PPPOEAC=N,PPPOESUB=N,LSVID=30,ENCAP=ETHERNETV2,DOS=Y,STP=OFF,STPCOST=100,STPPRIO=128,DIRN=BOTH,STAGTYPE=CTAG_8100,PORTTYPE=EDGE,TRFPROF=SIP_20MS3WAY,BCKPROF=SIP_20MS3WAY,PATH=WKG,CVID=UNTAGGED,SVID=30,PRIO=0,RCVID=NONE,RXETHBWPROF=NONE,TXETHBWPROF=NONE,MCASTPROF=NONE"
					if ( $line =~ m!(N\d+-\d+-\d+-\d+-\d+-SIP\d+):.*:VLAN=(\w+),BRIDGE=(\w+-\d+-\w+),ARP=(\w+),DHCPDIR=(\w+),OPT82ACT=(\w+),IGMP=(\w+),PPPOEAC=(\w+),PPPOESUB=(\w+),LSVID=(\w+),ENCAP=(\w+),DOS=(\w+),STP=(\w+),STPCOST=(\w+),STPPRIO=(\w+),DIRN=(\w+),STAGTYPE=(\w+),PORTTYPE=(\w+),TRFPROF=(\w+),BCKPROF=(\w+),PATH=(\w+),CVID=(\w+),SVID=(\w+),PRIO=(\w+),RCVID=(\w+),RXETHBWPROF=(\w+),TXETHBWPROF=(\w+)(,|")!)
					{
